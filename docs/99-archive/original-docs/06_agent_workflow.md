...
## Production Agent Workflow Update

Agent workflow must sit on top of deterministic indexed data. It should not replace scanner, parser, resolver, graph validation, or index activation.

### Bounded Workflow By Question Type

Do static routing before dynamic planning.

| Question type | Default workflow |
| --- | --- |
| Exact file question | file lookup -> evidence |
| Exact symbol question | symbol lookup -> code chunk -> references when available |
| Exact endpoint question | endpoint lookup -> handler relation -> evidence |
| Architecture overview | architecture model -> layer evidence -> docs/files |
| Flow tracing | endpoint lookup -> graph path traversal -> path evidence |
| Impact analysis | target validation -> reverse graph traversal -> tests/endpoints -> risk |
| Debugging | keyword/search -> config/code evidence -> related graph context |

Use dynamic planning only when bounded routing cannot gather enough evidence.

### Graph Path Tool Contract

Flow and impact tools should return path objects, not only isolated nodes:
...
Each edge in the path must include:
...
### Agent Use Of Inferred Data

Rules:

- parser and resolver evidence can support direct claims;
- heuristic and LLM-inferred graph data must be labeled;
- inferred data can guide investigation, but should not be presented as confirmed without supporting evidence;
- insufficient evidence is better than an unsupported explanation.
...
Trace must not store raw secrets or private credentials.
