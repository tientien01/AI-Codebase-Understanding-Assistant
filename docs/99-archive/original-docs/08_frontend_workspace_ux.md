...
## 14. Production UX Additions

These additions support the P0-P2 production roadmap.

### 14.1 Index Quality Panel

Workspace and Index Jobs pages should expose index quality, not only indexed/not indexed.

Required content:

```text
Index Quality
- 238 / 240 files processed
- 11 unresolved imports
- 4 parser warnings
- 2 unsupported files
- 8 graph orphan nodes
...
- open failed/skipped files;
- compare with previous index when available;
- re-index.

### 14.2 Capability Readiness UI

Pages should read capability readiness and render accurate states:

- Code Explorer depends on `code_explorer`.
- Search page depends on `keyword_search` and optionally `semantic_search`.
- Graph page depends on `graph`.
- Assistant depends on `chat`.
- Architecture view depends on `architecture`.
- Guided Tours depend on `guided_tours`.

If one capability fails, do not block unrelated ready capabilities.

### 14.3 Relationship Provenance In Graph View

When a user selects a graph edge, show:

```text
Relation: calls
Source: AST call extraction
Resolved by: Python symbol resolver
Confidence: 0.94
Support: direct
Evidence: auth/router.py:31
```

Provenance labels:

- Confirmed from parser.
- Resolved from imports.
- Produced by framework rule.
- Inferred from graph proximity.
- Generated summary based on cited files.

Do not show all relation types as equally reliable.

### 14.4 Architecture View

Add a dedicated architecture surface separate from the raw graph.
...
- domain flows when available;
- file membership;
- reason for classification;
- confidence/provenance.

Architecture view should help a newcomer understand the project shape without reading the entire graph.

### 14.5 Guided Tours

Replace a single reading path with multiple guided tours.
...
- linked evidence or signals;
- action to open file, graph node, or ask assistant.

### 14.6 Incremental Update Summary

After re-index, show what changed:
...
- inspect changed endpoints;
- inspect changed graph edges;
- re-run stale chat question when available.

### 14.7 P2 Product Surfaces

These are advanced and should remain behind stable in-development states until implemented:

- Domain or business-flow view.
- Persona-specific tours for newcomer, reviewer, maintainer, and product/domain user.
- Architecture comparison between index versions.
- Deep graph reviewer report.
