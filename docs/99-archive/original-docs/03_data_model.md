...
## Production Data Model Update

This section extends the target data model for a production-oriented implementation. Existing entities remain valid, but new implementation should add these concepts before scaling indexing, graph, and incremental rebuilds.

### Canonical Identity Rule

Use two identities for graph and code intelligence records:

- database ID: internal row identity, may use `node_<uuid>`, `sym_<uuid>`, or numeric primary keys;
- canonical key: stable semantic identity used for graph merge, version comparison, citations, and incremental rebuild.

Examples:

```text
file:backend/app/auth/router.py
class:backend/app/auth/service.py:AuthService
function:backend/app/auth/service.py:AuthService.login
endpoint:POST:/api/auth/login
component:frontend/src/pages/LoginPage.tsx:LoginPage
config:docker-compose.yml:services.api
table:users
```

Rules:

- canonical keys must be deterministic;
- canonical key generation must live in one utility module;
- graph edges should reference canonical keys or store canonical source/target fields in addition to database IDs;
- canonical keys must be version-comparable across indexes;
- line numbers must not be the only stable identity for symbols.

### IndexVersion

Purpose:

- represent one complete or attempted build of a repository index;
- support blue-green activation;
- make partial build state observable.
...
- `repository_id`
- `version`
- `status`: `building`, `ready`, `ready_with_warnings`, `failed`, `cancelled`, `superseded`
- `base_index_version`
- `source_revision`
...
Repository should point to an active index version:

- `repositories.active_index_version`

Do not switch `active_index_version` until the new index passes atomic publish gates.

### IndexArtifact

Purpose:

- record generated artifacts for debugging, replay, and validation.
...
- `content_hash`
- `size_bytes`
- `retention_class`: `permanent`, `debug`, `temporary`
- `created_at`
...
- drive incremental indexing decisions.
...
- store output of symbol, import, endpoint, test, config, and model resolvers.
...
- keep graph extraction auditable before normalization.
...
- make repository-level quality visible and testable.
...
Critical validation issues block activation. Warnings may allow `ready_with_warnings`.

### CapabilityReadiness

Capability readiness can be stored as JSON on `IndexVersion`, and may later become a table if filtering is needed.
...
- replace a single reading path with explicit learning paths.
...
### Data Lifecycle Rules For Production Indexing

1. `IndexVersion` is created in `building`.
2. Derived records are written for the new `index_version`.
3. Artifacts are written and registered.
4. Validation issues are recorded.
5. Capability readiness is computed.
6. The new version becomes `ready` or `ready_with_warnings`.
7. Repository active version is switched in one transaction.
8. Old versions are retained or cleaned according to retention policy.

If the build fails, the previous active version remains active.
