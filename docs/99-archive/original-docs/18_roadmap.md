...
```text
Your production web app architecture
+ deterministic indexing pipeline
+ graph artifact, provenance, validation
+ guided learning experience inspired by Understand Anything
```

Do not copy a plugin-first or autonomous multi-agent indexing design. Keep the backend, database, evidence model, and deterministic tools as the product foundation.

### P0 - Canonical Index Foundation

Goal:

- make indexing deterministic, inspectable, versioned, and safe to activate.

Deliverables:

- `IndexVersion` entity and active index pointer.
- Canonical repository scan inventory.
- Canonical keys for files, symbols, endpoints, graph nodes, and graph edges.
- Parser/resolver separation.
- Resolved reference model.
- Graph candidate model.
- Graph assembly and normalization.
- Repository-level validation report.
- Capability readiness status.
- Fingerprints and change classification.
- Atomic index activation.
- Index artifacts and manifest.

Exit criteria:

- failed new index does not replace previous active index;
- every graph edge has provenance;
- every indexable file is accounted for in validation;
- incremental plan includes affected files, dependents, and relink targets;
- frontend can show index quality and capability readiness.

### P1 - Strong Understanding Product

Goal:

- turn indexed code into a product that teaches users how the repository works.
...
- guided tours;
- relationship provenance UI;
- index comparison API and UI;
- index quality dashboard;
- claim-level citation foundation.

Exit criteria:

- a newcomer can follow a guided tour from entrypoint to core logic;
- a reviewer can inspect why a graph relation exists;
- an index update shows what changed and what was reused;
- architecture view explains layer membership with provenance.

### P2 - Advanced Product Differentiators

Goal:

- add optional semantic and agentic features without weakening deterministic foundations.

Deliverables:

- semantic batch planner;
- optional LLM architecture reviewer;
- domain/business-flow graph;
- persona-specific tours;
- architecture change comparison;
- automatic post-commit or watch-mode updates;
- deep multi-hop investigation over validated graph paths.

Exit criteria:

- inferred semantic output is labeled and validated;
- deterministic features remain usable when LLM providers fail;
- domain views and persona tours never invent unsupported relations;
- architecture comparison can explain meaningful changes between versions.

### Recommended Implementation Order

1. P0 data model and storage artifacts.
2. P0 scanner inventory and canonical key utilities.
3. P0 parser/resolver split.
4. P0 graph candidate, assembly, normalization, and validation.
5. P0 index version activation and capability readiness.
6. P0 incremental fingerprints and affected-set policy.
7. P1 index quality and provenance UI.
8. P1 architecture view and guided tours.
9. P1 graph path evidence and claim citations.
10. P2 semantic batching, domain view, persona tours, and deep agent features.

Agentic workflow should deepen after P0 is stable. Do not prioritize LangGraph orchestration before canonical indexing, graph validation, evidence, and atomic activation are reliable.
