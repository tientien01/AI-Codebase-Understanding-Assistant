...
### U001 - Import project bang folder, zip, hoac local path

| Thuoc tinh | Noi dung |
| --- | --- |
| Use case ID | U001 |
| Use Case | Import project bang folder, zip |
...
| Pre-Condition | Backend va frontend dang chay. Source ton tai, co quyen doc, va khong nam trong khu vuc bi cam cua he thong. |
| Result | He thong nhan source, tao import session hoac repository draft, va chuyen sang preview/indexing tuy theo flow duoc chon. |
...
| Alternative Scenarios | Neu user muon index ngay, he thong co the bo qua preview chi khi cau hinh cho phep.<br><br>Neu source la zip, he thong giai nen vao storage tam truoc khi scan. |
...
| Brief Description | Cho phep nguoi dung import public hoac private GitHub repository vao he thong de phan tich codebase. |
| Actor | Developer, reviewer, hoac tech lead co repository tren GitHub. |
| Pre-Condition | User co GitHub URL hop le. Voi private repo, user co token hoac co che xac thuc duoc ho tro. |
...
| Alternative Scenarios | User nhap token cho private repo.<br><br>User chon branch/tag khac.<br><br>User import repo public khong can token. |
...
| Non-Functional Constraints | Token khong duoc log hoac hien lai. Repo metadata phai luu branch/commit de phuc vu stale index va citation versioning. |
...
| Result | User thay status ro rang: queued, scanning, parsing, chunking, embedding, graph building, completed, completed with warnings, hoac failed. |
...
| Non-Functional Constraints | Status khong duoc bao thanh cong khi job chua luu xong du lieu. Loi file-level khong nen lam crash toan bo job neu co the bo qua hop ly. |
...
| Alternative Scenarios | Project tham chieu local path nen he thong khong xoa source goc, chi xoa metadata/index.<br><br>Project copy source vao managed storage thi co the xoa source copy. |
...
| Result | Repository record va du lieu lien quan duoc xoa theo chinh sach storage. UI khong con hien project. |
| Main Scenario | Buoc 1: User mo menu phu tren project card hoac Project Settings.<br><br>Buoc 2: User chon Delete.<br><br>Buoc 3: UI hien xac nhan, giai thich du lieu nao se bi xoa.<br><br>Buoc 4: User xac nhan.<br><br>Buoc 5: Backend xoa repository, index data, embeddings, graph, jobs, chat history, source copy neu thuoc managed storage.<br><br>Buoc 6: UI refresh Projects va hien success. |
| Alternative Scenarios | User huy xac nhan thi khong xoa gi.<br><br>Project dang indexing thi he thong dung/chan job truoc khi xoa tuy chinh sach. |
| Exception Flows | **Project not found:** Backend tra 404 va frontend refresh danh sach.<br><br>**Xoa database thanh cong nhung storage loi:** UI hien warning va log cleanup task.<br><br>**User dang o workspace project bi xoa:** Dieu huong ve Projects. |
...
| Brief Description | Mo rong san pham thanh multi-user bang dang nhap va phan quyen project. Day la use case tuy chon cho phase sau, khong bat buoc neu san pham la local single-user app. |
...
| Actor | Developer moi, developer bao tri, reviewer, nguoi demo. |
| Pre-Condition | Project da duoc index. He thong co cac tool noi bo nhu search, file detail, symbol detail, endpoint lookup, graph lookup, impact analysis, evidence viewer. Neu dung LangGraph, workflow agent da duoc cau hinh. |
...
### U033 - Dieu phoi tool bang LangGraph hoac agent workflow

| Thuoc tinh | Noi dung |
| --- | --- |
| Use case ID | U033 |
| Use Case | Dieu phoi tool bang LangGraph hoac agent workflow |
| Brief Description | Su dung LangGraph hoac mot workflow engine tuong duong de dieu phoi cac buoc plan, retrieve, graph lookup, verify, va answer thay vi viet if/else roi rac. |
...
| Main Scenario | Buoc 1: User gui cau hoi trong Chat.<br><br>Buoc 2: LangGraph workflow bat dau tu Intent Router.<br><br>Buoc 3: Planner node tao ke hoach va quyet dinh node tiep theo.<br><br>Buoc 4: Search/Graph/Endpoint/Impact nodes truy xuat du lieu.<br><br>Buoc 5: Evidence Merge node hop nhat va loai trung evidence.<br><br>Buoc 6: Verifier node kiem tra evidence co du de tra loi khong.<br><br>Buoc 7: Answer Generator node tao cau tra loi cuoi cung kem citation. |
| Alternative Scenarios | Neu project chua can LangGraph, co the bat dau bang workflow engine don gian nhung van giu cung concept node/state/tool.<br><br>Neu cau hoi chi can search don gian, workflow co the di tat qua Search -> Answer. |
...
1. Import source tu folder, zip, local path, va GitHub public/private theo pham vi duoc ho tro.
...
- Warnings allow `ready_with_warnings`.
...
- Code Explorer, Search, Graph, Chat, Architecture, and Guided Tours can have separate readiness.
