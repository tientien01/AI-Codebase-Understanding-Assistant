...
Status: Accepted supporting risk specification  
Authority: Detailed risk context; control IDs and L3 gates are owned by `../threat-model.md`  
Owner: Security owner  
Dependencies: `../threat-model.md`, `../../02-system-architecture/failure-model.md`  
Related source: `../../14-implementation-baseline/`  
Related tests: adversarial import, parser isolation, prompt injection, authorization, deletion, and provider fault suites  
...
- The product is local-first, while L3 production uses the accepted single-node topology.
- PostgreSQL is authoritative in production; SQLite is local/test only.
- Deterministic retrieval is mandatory; semantic/vector storage is optional and benchmark-triggered behind a provider abstraction.
- LLM and embedding providers are configurable.
- Production v1 supports constrained public GitHub import only; private/arbitrary Git requires new requirements and threat/ADR/task approval.
- Multi-user sharing/RBAC is deferred, but single-operator authentication, repository authorization, audit, quotas and TLS are mandatory for L3.

## Production Indexing Risks

### Canonical ID Drift

Risk:

- canonical keys change between index versions even when the underlying entity did not change.

Mitigation:

- generate canonical keys from stable semantic identity, not line numbers;
- centralize key generation;
- test line-shift stability;
- store old/new key comparison during index comparison.

Acceptance:

- unchanged files and symbols keep canonical keys after harmless edits.

### Incremental Divergence

Risk:

- incremental index result diverges from full rebuild.

Mitigation:

- compare incremental and full rebuild on fixtures;
- expand affected set to dependents, endpoint chains, tests, and semantic batches;
- keep fingerprints by content, structure, public API, dependency, and chunk.

Acceptance:

- fixture incremental rebuild matches full rebuild for canonical graph and search behavior.

### Graph Provenance Loss

Risk:

- graph edges remain but their source evidence is lost after merge or normalization.

Mitigation:

- require provenance on graph candidates;
- merge provenance arrays;
- block evidence-based labels when provenance is missing.

Acceptance:

- every active graph edge can explain origin and evidence support.

### Incomplete Index Activation

Risk:

- new index becomes active before graph, search, or validation is complete.

Mitigation:

- use `IndexVersion` lifecycle;
- publish atomically;
- keep previous active version until new version is ready;
- compute capability readiness.

Acceptance:

- failed or cancelled index never serves mixed old/new data.

### Semantic Batch Inconsistency

Risk:

- optional semantic enrichment produces inconsistent summaries across related files.

Mitigation:

- plan semantic batches by dependency/module boundaries;
- validate LLM output schema;
- label semantic output as inferred;
- keep deterministic graph usable without semantic enrichment.

Acceptance:

- semantic failure degrades architecture/tours, not code explorer or deterministic graph.

### Architecture Classification Instability

Risk:

- small code changes cause layers, modules, or tours to change unpredictably.

Mitigation:

- use stable layer IDs;
- prefer deterministic framework/path rules;
- compare architecture artifacts between versions;
- require reason and provenance for layer assignment.

Acceptance:

- minor unrelated edits do not rewrite the entire architecture view.
