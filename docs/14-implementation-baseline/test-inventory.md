...
The repository contains 112 pytest tests: 60 existing behavior tests, 33 PostgreSQL migration, repository, job-state/resilience, and Redis queue tests, and 19 immutable artifact/configuration tests. The canonical verified commands are:
...
- Lease/heartbeat, bounded retry, durable cancellation, stale-generation fencing, and Redis worker-loss recovery are covered by JOB-004. Immutable filesystem artifacts and terminal manifest publication are covered by IDX-001; typed stage checkpoints and atomic activation remain later indexing tasks.
