...
- Frontend coverage includes 45 Vitest tests across routing/application, API client, import, server-state policy, bounded graph and UI-004 workspace component suites. UI-004 also adds six deterministic Chromium Playwright tests, axe serious/critical checks on its three journeys, reduced-motion verification and Small/Medium/Large graph observations. Broader MSW API contracts and accepted release performance budgets remain absent.
...
| `npm.cmd run build` | Passed after clean `npm ci`: 131 modules; JS 374.30 kB (114.36 kB gzip), CSS 36.15 kB (8.59 kB gzip) |
