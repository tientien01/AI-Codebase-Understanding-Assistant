...
| Search/retrieval | Implemented typed baseline with optional versioned dense artifact | Owned typed retrievers feed content-addressed rank-only weighted RRF. The accepted local `embeddinggemma` benchmark now backs an immutable dense artifact and explicit semantic query adapter bound to repository/index, model digest, dimension, preprocessing and chunk hashes. Incompatible/provider-failed dense state falls back to sparse. Production worker composition, automatic activation/configuration, ANN scale, answer benchmarks and production thresholds remain absent |
...
| Assistant | Partial validated stateful baseline | Typed bounded routing, question-specific sufficiency, one controlled repair and selected/current claim-citation validation feed exact validated provider source context plus atomic redacted conversation/claim/citation/structured-trace persistence. Repository-owned bounded list/replay APIs, retained frontend conversation identity, refresh/deep-link replay and an eight-message/1,000-token non-evidentiary memory projection are tested. Optional page/file/line/symbol request context remains visible and validated; a loopback-only native Ollama transport is protocol-tested with deterministic fallback. Graph/API context, real-model qualification, retention execution and accepted agent thresholds remain absent |
| Real LLM provider | Optional adapter implemented; quality unverified | The Ollama adapter validates loopback origin/model/timeout, checks installed-model readiness and sends whole selected source spans through non-streaming JSON chat with citation validation and deterministic fallback. Tests use a deterministic fake transport; no live-model quality, latency, memory or release threshold is verified |
