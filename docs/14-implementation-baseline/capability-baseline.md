...
| Assistant | Partial validated baseline | Typed bounded routing, question-specific sufficiency, one controlled repair and selected/current claim-citation validation now feed exact validated provider source context plus atomic redacted conversation/claim/citation/structured-trace persistence with owned replay; no semantic entailment benchmark, public history API, automated retention executor or accepted agent threshold |
