...
from app.api.dependencies import get_indexing_use_cases
...
from app.services.application.use_cases import IndexingUseCases
...
def start_indexing(
    ...
    service: IndexingUseCases = Depends(get_indexing_use_cases),
) -> IndexResponse:
    result = service.start_indexing_background(repository_id, request.force_reindex if request else False)
...
def get_index_status(
    ...
    return service.get_index_status(repository_id)
...
def list_indexing_jobs(
    ...
    return service.list_indexing_jobs(repository_id)


@router.post("/{repository_id}/index/jobs/{job_id}/pause", response_model=IndexJobControlResponse)
def pause_indexing_job(
    repository_id: str,
    job_id: str,
    service: IndexingUseCases = Depends(get_indexing_use_cases),
) -> IndexJobControlResponse:
    result = service.pause_indexing_job(repository_id, job_id)
...
def resume_indexing_job(
    repository_id: str,
    job_id: str,
    service: IndexingUseCases = Depends(get_indexing_use_cases),
) -> IndexJobControlResponse:
    result = service.resume_indexing_job(repository_id, job_id)
    return IndexJobControlResponse(**result)
...
def cancel_indexing_job(
    ...
    result = service.cancel_indexing_job(repository_id, job_id)
    return IndexJobControlResponse(**result)
...
def get_index_warnings(
    ...
    return service.get_index_warnings(repository_id, job_id)
...
def get_skipped_files(
    ...
    return service.get_skipped_files(repository_id, job_id)
...
def get_failed_files(
    ...
    return service.get_failed_files(repository_id, job_id)
...
def get_staleness(
    ...
    return service.get_staleness(repository_id)
